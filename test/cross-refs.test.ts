...
async function getReferences(): Promise<ReferenceDescription[]> {
    const datatypeDoc: LangiumDocument<Definitions> = await parseDocument(services, datatypeFile);
    const referencingDoc: LangiumDocument<Definitions> = await parseDocument(services, referencingFile);

    await services.shared.workspace.DocumentBuilder.build([referencingDoc, datatypeDoc]);
    const model = datatypeDoc.parseResult.value;
