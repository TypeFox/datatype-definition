...
import { Definitions, isType, PackageDeclaration, isPackageDeclaration } from './generated/ast';

export class DatatypeDefinitionScopeComputation extends DefaultScopeComputation {

    qualifiedNameProvider: DatatypeDefinitionQualifiedNameProvider;

    constructor(services: DatatypeDefinitionServices) {
        super(services);
    ...
    override async computeLocalScopes(document: LangiumDocument, cancelToken = CancellationToken.None): Promise<PrecomputedScopes> {
        const model = document.parseResult.value as Definitions;
        const scopes = new MultiMap<AstNode, AstNodeDescription>();
        await this.processContainer(model, scopes, document, cancelToken);
        return scopes;
    }

    protected async processContainer(container: Definitions | PackageDeclaration, scopes: PrecomputedScopes, document: LangiumDocument, cancelToken: CancellationToken): Promise<AstNodeDescription[]> {
